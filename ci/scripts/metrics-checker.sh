...
checkServiceYAML(){
    SERVICE_NAME=$1
    if grep -q "include \"$SERVICE_NAME.annotations\" ." ./charts/$SERVICE_NAME/templates/service.yaml &&
       grep -q "\"$SERVICE_NAME.prometheus\"" ./charts/$SERVICE_NAME/templates/_helpers.tpl; then
        ...
        echo -e "What is needed:"
        echo -e "include \"$SERVICE_NAME.annotations\" . in service.yaml"
...
checkPomXML(){
    ...
    grep -B2 -A7 "<artifactId>micrometer-core</artifactId>" ./eric-oss-scoping-poc-app/pom.xml | sed 's/^[ \t]*//;s/[ \t]*$//' > .bob/var.scrapedLine || true;
...
checkCoreApplicationJAVA(){
    ...
    echo -e "public WebClient webClient(final WebClient.Builder webClientBuilder)" > .bob/var.compareToLine;
      echo -e "return webClientBuilder.build()" >> .bob/var.compareToLine;

    find ./eric-oss-scoping-poc-app/src/main/java -regex '.*Application.java' -exec grep -o 'public WebClient webClient(final WebClient.Builder webClientBuilder)' {} \; > .bob/var.scrapedLine || true;
    find ./eric-oss-scoping-poc-app/src/main/java -regex '.*Application.java' -exec grep -o 'return webClientBuilder.build()' {} \; >> .bob/var.scrapedLine || true;
